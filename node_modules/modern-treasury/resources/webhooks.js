...
class Webhooks extends resource_1.APIResource {
    ...
    getSignature(payload, opts) {
        ...
        if (key == null) {
            throw new Error("The webhook key must either be set using the env var, MODERN_TREASURY_WEBHOOK_KEY, on the client class, new ModernTreasury({ webhookKey: '123' }) or passed to this function");
        }
        if (!key) {
    ...
    validateSignature(payload, headers, opts) {
        ...
        const expectedSignature = typeof headers === 'string' ? headers : (0, headers_1.getRequiredHeader)(headers, 'X-Signature');
